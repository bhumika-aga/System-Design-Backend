...
class VideoProcessingWorkflowImpl implements VideoProcessingWorkflow {
    ...
    private final VideoActivities activities = Workflow.newActivityStub(
        VideoActivities.class,
        OPTIONS);
    ...
    public void process(String videoId) {
        ...
        Promise<Void> thumbnails = Async.procedure(
            activities::generateThumbnails,
            encodedPath);
        Promise<Void> transcript = Async.procedure(
            activities::generateTranscription,
            encodedPath);
