...
class TokenBucketRateLimiter:
    ...
    def acquire(self) -> bool:
        ...
        """
        result = self.r.eval(
            lua_script, 1, self.key, self.capacity, self.refill_rate, now)
        return bool(result)


# Usage inside a Celery task
limiter = TokenBucketRateLimiter(
    r, "ratelimit:resend", capacity=100, refill_rate=10)
...
def send_email_task(self, email, token):
    if not limiter.acquire():
        ...
        # retry every 1s up to 60 times
        raise self.retry(countdown=1, max_retries=60)
